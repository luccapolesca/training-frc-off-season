...
public class Robot extends TimedRobot {
  ...
  public void robotInit() {
    ...
    // Creer les petits robots
    smallBots = new SmallBot(xDeplacementPrecedent, degreePrecedent, kDefaultPeriod);
    smallBots = null;
  }
  ...
  public void robotPeriodic() {
    ...
    field.getObject("robot").setPose(0, yDeplacement, Rotation2d.fromDegrees(degree));
    ...
    for (int i = 0; i < enemyies.length; i++) {
      ...
      // double xDistance = Math.abs(smallBots.x - enemyies[i].x);
      // boolean xSeTouche = xDistance < 0.5;

      // double yDistance = Math.abs(smallBots.y - enemyies[i].y);
    ...
    // Des petites robots
    boolean shootBullet = keyA.getRawButton(2);
    SmartDashboard.putBoolean("petitRobot", shootBullet);
    ...
    if (smallBots != null){
      smallBots.x = smallBots.x + smallBots.xSpeed * 2;

      field.getObject("smallBots").setPose(smallBots.x, yDeplacement, Rotation2d.fromDegrees(degree));

      
      
      if (smallBots.x > 16) {
        smallBots = null;
        field.getObject("smallBots").setPose(199, 0, Rotation2d.fromDegrees(degree));
      }
    }
  }
